...
if [ "${GIT_LOCATION}" = "EXTERNAL" ] ; then
    ...
    : ${GIT_PORTS_REPO=https://github.com/librebsd/ports.git}
fi

: ${GIT_FREEBSD_CACHE="file:///librebsd-build/trueos.git"}
: ${GIT_FREEBSD_BRANCH=feature/unified_freebsd}
: ${GIT_FREEBSD_REPO=git@gitserver:/git/repos/librebsd-build/trueos.git}

: ${GIT_PORTS_CACHE="file:///librebsd-build/ports.git"}
: ${GIT_PORTS_BRANCH=librebsd/9.1-stable-b}
: ${GIT_PORTS_REPO=git@gitserver:/git/repos/librebsd-build/ports.git}
...
if [ -e "${GIT_PORTS_CACHE##file://}" ]; then
    ...
    echo "no local mirror, to speed up builds we suggest doing"
    echo "'git clone --mirror https://github.com/librebsd/ports.git into ${HOME}/librebsd/git/ports.git"
fi
...
main()
{
	...
	#
	# You must be root to build LibreBSD
	#
	set -e
	if $BUILD
	then
